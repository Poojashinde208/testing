...
var defaultCustomisations = __spreadProps(__spreadValues({}, defaultIconCustomisations), {
  preserveAspectRatio: ""
});
function getCustomisations(node) {
  const customisations = __spreadValues({}, defaultCustomisations);
...
function renderSPAN(data, icon, useMask) {
  ...
  const html = iconToHTML$1(body, __spreadProps(__spreadValues({}, renderAttribs), {
    width: icon.width + "",
    height: icon.height + ""
  }));
...
function renderIcon(parent, state) {
  ...
  switch (mode) {
    case "svg":
      ...
      break;
    default:
      node = renderSPAN(renderData, __spreadValues(__spreadValues({}, defaultIconProps), iconData), mode === "mask");
  }
